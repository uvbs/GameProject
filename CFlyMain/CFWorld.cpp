...
namespace CFly
{
	...
	World* createWorld( HWND hWnd , int w, int h , int cDepth , bool fullscreen )
	{
		return WorldManager::getInstance().createWorld( hWnd , w , h , cDepth , fullscreen );
	}
	...
	World::World( HWND hWnd , int w, int h , int cDepth , bool fullscreen ) 
		:mhWnd( hWnd )
		,mD3dDevice( NULL )
		,mFont( nullptr )
	{
		...
		if ( !mRenderSystem->init( mhWnd , w , h , fullscreen ) )
	...
	World* WorldManager::createWorld( HWND hWnd , int w, int h , int cDepth , bool fullscreen )
	{
		World* world;
		try
		{
			world = new World( hWnd , w , h , cDepth , fullscreen );
