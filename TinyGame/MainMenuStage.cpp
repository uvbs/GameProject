...
{
	INFO( "Planet Test"  , Planet::TestStage ), 
...
StageInfo gDev3Group[] =
{
	...
	INFO( "Phy2D Test" , Phy2D::CollideTestStage ) ,
	INFO( "Phy2D Test2" , Phy2D::WorldTestStage ) ,
...
StageInfo gDev2Group[] =
{
	...
	INFO( "Lighting Test"     , Lighting::TestStage ) ,
...
#define CREATE_BUTTON( ID , NAME )\
	...
	case UI_MAIN_GROUP:
		...
		CREATE_BUTTON( UI_GAME_DEV3_GROUP  , "Game Dev3"   );
	...
	case UI_GAME_DEV3_GROUP:
		for( int i = 0 ; i < ARRAY_SIZE( gDev3Group ) ; ++i )
		{
			CREATE_BUTTON( UI_GAME_DEV3_INDEX + i , LAN( gDev3Group[i].decl ) );
		}
		CREATE_BUTTON( UI_BACK_GROUP     , LAN("Back")          );
		break;
...
bool MainMenuStage::onEvent( int event , int id , GWidget* ui )
{
	...
	else
	{
		...
		{
			info = &gDev3Group[ id - UI_GAME_DEV3_INDEX ];
		}
