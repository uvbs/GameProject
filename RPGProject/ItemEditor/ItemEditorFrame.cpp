...
bool ItemEditorFrame::init()
{
...

	{
		...
		SRoleInfo* roleData = TRoleManager::getInstance().getRoleInfo( ROLE_HERO );

		ActorModelRes* modelRes = TResManager::getInstance().getActorModel( roleData->modelName.c_str() );
		CActorModel* modelComp = modelRes->createModel( mCFScene );

		mHeroEntity.addComponent( modelComp );

		PhyCollision* phyicalComp = mPhyWorld->createCollisionComponent( 
			modelRes->phyData.shape , PHY_ACTOR_OBJ , COF_ACTOR , COF_ALL );
		mHeroEntity.addComponent( phyicalComp );

		SNSpatialComponent* spatialComp = 
			new SNSpatialComponent( modelComp->getCFActor() , nullptr );
		spatialComp->setPhyOffset( modelRes->modelOffset );
		spatialComp->setPosition( Vec3D(0,0,0) );

		mHeroEntity.addComponent( spatialComp );
		mHeroEntity.addComponent( new ActorMovement( phyicalComp , 25 , 2 ) );
		mHeroEntity.addComponent( new AbilityPropComponent( roleData->propData ) );

		mHeroLogicComp = new CHero;
		mHeroEntity.addComponent( mHeroLogicComp );
	}
